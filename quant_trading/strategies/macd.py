...
class MACDStrategy(BaseStrategy):
    ...
    def analyze_macd_signal(
        ...
        if hist > 0 and last_hist <= 0:
            ...
            return {
                'action': 'BUY',
                'reason': 'Histogram turned positive (bullish crossover)',
        ...
        if hist < 0 and last_hist >= 0:
            ...
            return {
                'action': 'SELL',
                'reason': 'Histogram turned negative (bearish crossover)',
                'entry_price': close_price,
                'stop_loss': stop_loss,
                'confidence': min(0.8, abs(hist) / 0.3),
                'trade_type': 'OPEN'
    ...
    def should_trade(self, signal: Dict[str, Any], current_positions: Dict[str, Any]) -> Dict[str, Any]:
        if signal['confidence'] < 0.1:
            return {
                'should_trade': False,
                'reason': f'Confidence too low ({signal["confidence"]:.2f} < 0.1)',
        ...
        if signal['action'] == 'BUY' and current_pos > 0:
            return {
                'should_trade': False,
                'reason': f'Existing long position; skip additional buy (current: {current_pos} shares)',
                'details': {'current_position': current_pos, 'signal_action': signal['action']}
            }
        if signal['action'] == 'SELL' and current_pos < 0:
            return {
                'should_trade': False,
                'reason': f'Existing short position; skip additional sell (current: {current_pos} shares)',
                'details': {'current_position': current_pos, 'signal_action': signal['action']}
            }

        return {
            'should_trade': True,
            'reason': 'Passed trade condition checks',
    ...
    def analyze_position_and_signal(
        ...
        if base_signal['action'] == 'BUY':
            if current_pos < 0:
                return {
                    'action': 'BUY',
                    'reason': f'Close short ({abs(current_pos)} shares) + open long - {base_signal["reason"]}',
                    'entry_price': close_price,
            ...
            return {
                'action': 'HOLD',
                'reason': f'Existing long position ({current_pos} shares); no additional entry',
                'entry_price': close_price,
                'stop_loss': None,
                'confidence': 0.0,
                'trade_type': 'NONE'
        ...
        if base_signal['action'] == 'SELL':
            if current_pos > 0:
                return {
                    'action': 'SELL',
                    'reason': f'Close long ({current_pos} shares) + open short - {base_signal["reason"]}',
                    'entry_price': close_price,
            ...
            return {
                'action': 'HOLD',
                'reason': f'Existing short position ({abs(current_pos)} shares); no additional entry',
                'entry_price': close_price,
                'stop_loss': None,
                'confidence': 0.0,
                'trade_type': 'NONE'
